...
describe('setIndefiniteWhitelistStatus', function () {
  ...
  context('Sender is the manager address', function () {
    it('sets indefinite whitelist status', async function () {
      ...
      // Whitelist indefinitely
      await expect(
        rrpBeaconServer.connect(roles.manager).setIndefiniteWhitelistStatus(beaconId, roles.beaconReader.address, true)
      )
        .to.emit(rrpBeaconServer, 'SetIndefiniteWhitelistStatus')
      ...
      // Revoke indefinite whitelisting
      await expect(
        rrpBeaconServer.connect(roles.manager).setIndefiniteWhitelistStatus(beaconId, roles.beaconReader.address, false)
      )
        .to.emit(rrpBeaconServer, 'SetIndefiniteWhitelistStatus')
      ...
      context('Encoded data length is correct', function () {
          ...
          context('Data fresher than beacon', function () {
            ...
            context('Data older than 1 hour', function () {
              it('reverts', async function () {
                ...
                expect(testUtils.decodeRevertString(staticCallResult.callData)).to.equal('Fulfillment stale');
                await expect(
                  airnodeRrp
                    .connect(sponsorWallet)
                    .fulfill(
            ...
            context('Data more than 1 hour from the future', function () {
              it('reverts', async function () {
                ...
                expect(staticCallResult.callSuccess).to.equal(false);
                expect(testUtils.decodeRevertString(staticCallResult.callData)).to.equal('Fulfillment from future');
                await expect(
                  airnodeRrp
                    .connect(sponsorWallet)
                    .fulfill(
        ...
        context('Timestamp does not typecast successfully', function () {
          it('reverts', async function () {
            ...
            expect(staticCallResult.callSuccess).to.equal(false);
            expect(testUtils.decodeRevertString(staticCallResult.callData)).to.equal('Timestamp typecasting error');
            await expect(
              airnodeRrp
                .connect(sponsorWallet)
                .fulfill(
