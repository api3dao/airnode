...
describe('convertor', () => {
  it('Converts from OAS to OIS', () => {
    expect(convert('exampleSpecs/OAS.specs.json', 'templates/conversion/oas@3.0.0->ois@pre-alpha.json')).toEqual({
      valid: false,
      messages: [
  ...
  it('Converts from OIS to config', () => {
    expect(
      convert('exampleSpecs/OIS2.specs.json', 'templates/conversion/ois@pre-alpha->config@pre-alpha.json')
    ).toEqual({
      valid: true,
      messages: [],
