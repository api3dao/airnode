...
export async function deployServerless(providerIdShort) {
  ...
  await exec(`PROVIDER_ID_SHORT=${providerIdShort} AWS_REGION=$AWS_REGION npm run sls:deploy`);
...
export async function removeServerless(providerIdShort) {
  ...
  await exec(`PROVIDER_ID_SHORT=${providerIdShort} AWS_REGION=$AWS_REGION npm run sls:remove`);
