...
describe('callApi', () => {
  ...
  it('calls the adapter with the given parameters (with _relay_metadata set to v1 lowercase)', async () => {
    const spy = jest.spyOn(adapter, 'buildAndExecuteRequest') as any;
    spy.mockResolvedValueOnce({ data: { price: 1000 } });
  ...
  it('calls the adapter with the given parameters (with _relay_metadata set to V1 uppercase)', async () => {
    const spy = jest.spyOn(adapter, 'buildAndExecuteRequest') as any;
    spy.mockResolvedValueOnce({ data: { price: 1000 } });
    const parameters = { _type: 'int256', _path: 'price', from: 'ETH', _relay_metadata: 'V1' };
    const aggregatedCall = fixtures.buildAggregatedApiCall({ parameters });
