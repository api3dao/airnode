...
describe('callApi', () => {
  it('calls the adapter with the given parameters', async () => {
    ...
    const [logs, res] = await execution.callApi(aggregatedCall);
    expect(logs).toEqual([]);
  ...
  it('returns an error if no _type parameter is found', async () => {
    ...
    const [logs, res] = await execution.callApi(aggregatedCall);
    expect(logs).toEqual([
      { level: 'ERROR', message: "No '_type' parameter was found for Endpoint:endpointName, OIS:oisTitle" },
    ]);
    expect(res).toEqual({
  ...
  it('returns an error if the API call fails to execute', async () => {
    ...
    const [logs, res] = await execution.callApi(aggregatedCall);
    expect(logs).toEqual([
      { level: 'ERROR', message: 'Failed to call Endpoint:endpointName', error: new Error('Network is down') },
    ]);
    expect(res).toEqual({
