...
export async function fetchWalletDataByIndex(state: ProviderState<EVMProviderState>): Promise<LogsData<WalletDataByIndex>> {
  ...
  const log = logger.pend('INFO', `Pending requests: ${apiCalls.length} API call(s), ${withdrawals.length} withdrawal(s), ${walletDesignations.length} wallet designation(s)`);
