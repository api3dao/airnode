...
async function getWalletTransactionCount(address: string, options: FetchOptions): Promise<LogsData<TransactionCountByAddress | null>> {
  ...
  if (err || count === null) {
    const log = logger.pend('ERROR', `Unable to fetch transcation count for wallet:${address}`, err);
    return [[log], null];
  }
  return [[], { [address]: count }];
}

export async function fetchByAddress(addresses: string[], options: FetchOptions): Promise<LogsData<TransactionCountByAddress>> {
  // Fetch all transaction counts in parallel
  const promises = addresses.map((address) => getWalletTransactionCount(address, options));
