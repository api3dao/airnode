...
import { submitApiCall } from './transactions/api-calls';
import { submitWalletDesignation } from './transactions/wallet-designations';
import { submitWithdrawal } from './transactions/withdrawals';
...
export async function submit(state: ProviderState) {
  ...
  const promises = flatMap(walletIndices, (index) => {
    ...
    const txOptions: TransactionOptions = { gasPrice: state.gasPrice!, provider: state.provider, walletIndex: index };
  ...
  const sucessfulResponses = responses.filter((r) => !!r.data);

  const receipts = sucessfulResponses.map((r) => ({ id: r.id, type: r.type, transactionHash: r.data.hash }));
