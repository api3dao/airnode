...
function mapApiCalls(apiCalls: ClientRequest<ApiCall>[], authorizationsByEndpointId: AuthorizationByEndpointId): LogsErrorData<ClientRequest<ApiCall>[]> {
  const logsWithApiCalls: LogsErrorData<ClientRequest<ApiCall>>[] = apiCalls.map((apiCall) => {
    ...
    const log = logger.pend('WARN', `Client:${apiCall.requesterAddress} is not authorized to access Endpoint ID:${apiCall.endpointId} for Request ID:${apiCall.id}`);
    const updatedApiCall = {
      ...apiCall,
...
export function mergeAuthorizations(
  ...
  const updatedWalletDataWithLogs = walletIndices.reduce((acc, index) => {
