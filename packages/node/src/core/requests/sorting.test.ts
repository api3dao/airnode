...
describe('sortRequests', () => {
  it('sorts API calls by block number then by transaction hash', () => {
    const first = fixtures.requests.createApiCall({ metadata: { blockNumber: 100, transactionHash: '0x1' } });
    const second = fixtures.requests.createApiCall({ metadata: { blockNumber: 100, transactionHash: '0x2' } });
    const third = fixtures.requests.createApiCall({ metadata: { blockNumber: 101, transactionHash: '0x3' } });
    const fourth = fixtures.requests.createApiCall({ metadata: { blockNumber: 101, transactionHash: '0x4' } });
  ...
  it('sorts withdrawals by block number then by transaction hash', () => {
    const first = fixtures.requests.createWithdrawal({ metadata: { blockNumber: 100, transactionHash: '0x1' } });
    const second = fixtures.requests.createWithdrawal({ metadata: { blockNumber: 100, transactionHash: '0x2' } });
    const third = fixtures.requests.createWithdrawal({ metadata: { blockNumber: 101, transactionHash: '0x3' } });
    const fourth = fixtures.requests.createWithdrawal({ metadata: { blockNumber: 101, transactionHash: '0x4' } });
