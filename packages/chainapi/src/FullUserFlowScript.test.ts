...
describe('Full user flow', () => {
  ...
  beforeEach(async () => {
    ...
    clientContractFactory = new ethers.ContractFactory(clientContractArtifact.abi, clientContractArtifact.bytecode, signer);
  });
  ...
  it('works', async () => {
    ...
    tx = await contractAsRequesterAdmin.createTemplate(providerId, endpointId, ethers.BigNumber.from('500000'), '0x12341234');
    logs = await ethProvider.getLogs({address: contract.address});
    log = (logs.filter(log => log.transactionHash === tx.hash))[0];
