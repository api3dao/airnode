  ...
  describe('post deployment', async () => {
    ...
    describe('setMinimumLockingTime', async () => {
      ...
      context('caller is admin', async () => {
        it('is set', async () => {
          // when:
          await api3TokenLockExternal.setMinimumLockingTime(NEW_MINIMUM_LOCKING_TIME);
          // then:
          expect(await api3TokenLockExternal.minimumLockingTime()).to.equal(NEW_MINIMUM_LOCKING_TIME);
        });

        it('emits event', async () => {
          await expect(api3TokenLockExternal.setMinimumLockingTime(NEW_MINIMUM_LOCKING_TIME))
    ...
    describe('setLockAmount', async () => {
      ...
      context('caller is admin', async () => {
        it('is set', async () => {
          // when:
          await api3TokenLockExternal.setLockAmount(NEW_LOCK_AMOUNT);
          // then:
          expect(await api3TokenLockExternal.lockAmount()).to.equal(NEW_LOCK_AMOUNT);
        });

        it('emits event', async () => {
          await expect(api3TokenLockExternal.setLockAmount(NEW_LOCK_AMOUNT))
        ...
        context('new lock amount is zero', async () => {
          it('reverts', async () => {
            await expect(api3TokenLockExternal.setLockAmount(0)).to.revertedWith(Errors.ZeroAmount);
          });
        });
      });
    ...
    describe('setBlacklistStatus', async () => {
      context('caller is owner', async () => {
        it('is set', async () => {
          // when:
          await api3TokenLockExternal.setBlacklistStatus(bobClient.address, true);
          // then:
          expect(await api3TokenLockExternal.clientAddressToBlacklistStatus(bobClient.address)).to.equal(true);
        });

        it('emits event', async () => {
          await expect(api3TokenLockExternal.setBlacklistStatus(bobClient.address, true))
    ...
    describe('lock', async () => {
      ...
      context('caller is first time locking', async () => {
        ...
        it('is set', async () => {
          ...
          expect(await api3TokenLockExternal.tokenLocks(chainId, airnodeId, bobClient.address)).to.equal(1);
        });
    ...
    describe('unlock', async () => {
      ...
      context('caller is unlocking successfully', async () => {
        ...
        it('is set', async () => {
          ...
          expect(await api3TokenLockExternal.tokenLocks(chainId, airnodeId, bobClient.address)).to.equal(0);
        });
    ...
    describe('transfer', async () => {
      ...
      context('from one client to another', async () => {
        it('is set', async () => {
          ...
          const beforeBobWhitelistCount = await api3TokenLockExternal.tokenLocks(chainId, airnodeId, bobClient.address);
          const beforeCarolWhitelistCount = await api3TokenLockExternal.tokenLocks(
            chainId,
            airnodeId,
            carolClient.address
          );
          ...
          expect(await api3TokenLockExternal.tokenLocks(chainId, airnodeId, bobClient.address)).to.equal(
          ...
          expect(await api3TokenLockExternal.tokenLocks(chainId, airnodeId, carolClient.address)).to.equal(
      ...
      context('toClient address is blacklisted', async () => {
        it('reverts', async () => {
          // given:
          await api3TokenLockExternal.setBlacklistStatus(carolClient.address, true);
          // then:
          await expect(
            api3TokenLockExternal.connect(alice).transfer(chainId, airnodeId, bobClient.address, carolClient.address)
    ...
    describe('burn', async () => {
      ...
      context('successful burn of blacklisted client', async () => {
        ...
        it('burns tokens', async () => {
          ...
          const beforeWhitelistCount = await api3TokenLockExternal.tokenLocks(chainId, airnodeId, bobClient.address);

          // when:
          await api3TokenLockExternal.burn(chainId, airnodeId, bobClient.address, alice.address);
        ...
        it('emits event', async () => {
          await expect(api3TokenLockExternal.burn(chainId, airnodeId, bobClient.address, alice.address))
      ...
      context('burn target has zero locked amount', async () => {
        it('reverts', async () => {
          await expect(
            api3TokenLockExternal.burn(chainId, airnodeId, bobClient.address, unknown.address)
      ...
      context('client address is not blacklisted', async () => {
        it('reverts', async () => {
          // given:
          await api3TokenLockExternal.setBlacklistStatus(bobClient.address, false);
          // then:
          await expect(
            api3TokenLockExternal.burn(chainId, airnodeId, bobClient.address, alice.address)
