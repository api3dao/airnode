...
beforeEach(async () => {
  ...
  const airnodeFactory = await ethers.getContractFactory(
    "Airnode",
    roles.deployer
  );
...
describe("createRequester", function () {
  it("creates a requester record", async function () {
    ...
    const log = await verifyLog(airnode, tx, "RequesterCreated(uint256,address)", { admin: roles.requesterAdmin._address });
  ...
  it("assigns requester indices incrementally", async function () {
    ...
    const log1 = await verifyLog(airnode, tx1, "RequesterCreated(uint256,address)", { admin: roles.requesterAdmin._address });
    ...
    const log2 = await verifyLog(airnode, tx2, "RequesterCreated(uint256,address)", { admin: roles.requesterAdmin._address });
...
describe("updateRequesterAdmin", function () {
  context("If the caller is the requester admin", async function () {
    it("updates the requester admin", async function () {
      ...
      await expect(airnode.connect(roles.requesterAdmin).updateRequesterAdmin(ethers.BigNumber.from(1), roles.updatedRequesterAdmin._address))
  ...
  context("If the caller is not the requester admin", async function () {
    it("reverts", async function () {
      await airnode.connect(roles.requesterAdmin).createRequester(roles.requesterAdmin._address);
      await expect(airnode.connect(roles.randomPerson).updateRequesterAdmin(ethers.BigNumber.from(1), roles.updatedRequesterAdmin._address))
  ...
  describe("updateClientEndorsementStatus", function () {
    context("If the caller is the requester admin", async function () {
      it("updates the client endorsement status", async function () {
        ...
        // Endorse the client
        await expect(airnode.connect(roles.requesterAdmin).updateClientEndorsementStatus(ethers.BigNumber.from(1), roles.client._address, true))
        ...
        await expect(airnode.connect(roles.requesterAdmin).updateClientEndorsementStatus(ethers.BigNumber.from(1), roles.client._address, false))
          .to.emit(airnode, "ClientEndorsementStatusUpdated")
    ...
    context("If the caller is not the requester admin", async function () {
      it("reverts", async function () {
        await airnode.connect(roles.requesterAdmin).createRequester(roles.requesterAdmin._address);
        await expect(airnode.connect(roles.randomPerson).updateClientEndorsementStatus(ethers.BigNumber.from(1), roles.client._address, false))
          .to.be.revertedWith("Caller is not requester admin");
      });
    });
  });
