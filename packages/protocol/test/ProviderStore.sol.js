  ...
  context('If the parameters are correct', async function () {
    it('fulfills withdrawal', async function () {
      ...
      const gasEstimate = await airnode
        ...
        .estimateGas.fulfillWithdrawal(withdrawalRequestId, providerId, requesterInd, destination, { value: 1, gasLimit:500000 });
  ...
  context('If the parameters are incorrect', async function () {
    it('reverts', async function () {
      ...
      await expect(
        airnode
          .connect(designatedWallet)
          .fulfillWithdrawal(ethers.constants.HashZero, providerId, requesterInd, destination, { value: 1, gasLimit:500000 })
      ).to.be.revertedWith('No such withdrawal request');
      await expect(
        airnode
          .connect(designatedWallet)
          .fulfillWithdrawal(withdrawalRequestId, ethers.constants.HashZero, requesterInd, destination, { value: 1, gasLimit:500000 })
      ).to.be.revertedWith('No such withdrawal request');
      await expect(
        airnode
          .connect(designatedWallet)
          .fulfillWithdrawal(withdrawalRequestId, providerId, ethers.constants.Zero, destination, { value: 1, gasLimit:500000 })
      ).to.be.revertedWith('No such withdrawal request');
      await expect(
        airnode
          .connect(designatedWallet)
          .fulfillWithdrawal(withdrawalRequestId, providerId, requesterInd, ethers.constants.AddressZero, { value: 1, gasLimit:500000 })
      ).to.be.revertedWith('No such withdrawal request');
    });
  });
});
