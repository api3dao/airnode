...
contract SelectK {
  ...
  uint256 public constant SMALL_ARRAY_MAX_LENGTH = 16;
  ...
  function compute(int256[] memory arr, uint256 k) external pure returns (int256) {
  ...
  ///         respectively
  function compute2(int256[] memory arr, uint256 k) external pure returns (int256, int256) {
  ...
  function computeInPlace(int256[] memory arr, uint256 k) public pure returns (int256) {
    require(k <= arr.length - 1, "k must be a valid index in arr");
  ...
  /// @return a tuple containing the kth and (k+1)st smallest elements in `arr`,
  ///         respectively
  function compute2InPlace(int256[] memory arr, uint256 k) public pure returns (int256, int256) {
    require(k <= arr.length - 2, "k must be a valid index in arr");
    if (arr.length <= SMALL_ARRAY_MAX_LENGTH) {
  ...
  function quickSelect(
    ...
    idx2 = idx1 + 1;
    for (uint256 i = idx1 + 2; i < arr.length; i++) {
      if (arr[i] < arr[idx2]) {
        idx2 = i;
      }
    }
  ...
  /// @return the kth smallest element in `arr`
  function selectKsmallArray(int256[] memory arr, uint256 k) private pure returns (int256) {
    if (arr.length == 2) {
      condSwap(arr, 0, 1);
  ...
  function copy(int256[] memory arr) private pure returns (int256[] memory) {
