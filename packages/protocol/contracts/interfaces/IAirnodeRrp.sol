...
interface IAirnodeRrp is IAirnodeParameterStore, ITemplateStore {
  ...
  event ClientRequestFulfilled(bytes32 indexed airnodeId, bytes32 indexed requestId, uint256 statusCode, bytes data);

  event ClientRequestFailed(bytes32 indexed airnodeId, bytes32 indexed requestId);
