...
contract RequesterStore is IRequesterStore {
  ...
  modifier onlyRequesterAdmin(uint256 requesterIndex) {
    require(msg.sender == requesterIndexToAdmin[requesterIndex], "Caller not requester admin");
    _;
  }
  ...
  function createRequester(address admin) external override returns (uint256 requesterIndex) {
    ...
    emit RequesterCreated(requesterIndex, admin);
  }

  /// @notice Sets the requester admin
  /// @param requesterIndex Requester index
  /// @param admin Requester admin
  function setRequesterAdmin(uint256 requesterIndex, address admin)
    external
    override
    onlyRequesterAdmin(requesterIndex)
  {
    requesterIndexToAdmin[requesterIndex] = admin;
    emit RequesterUpdated(requesterIndex, admin);
  }
  ...
  function setClientEndorsementStatus(
    ...
    emit ClientEndorsementStatusSet(requesterIndex, clientAddress, endorsementStatus);
  }
}
