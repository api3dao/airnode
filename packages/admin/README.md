...
Derives the address of the wallet designated by an Airnode for a requester, which is called the sponsor wallet. The
`mnemonic` must be from the Airnode wallet and the `xpub` must belong to the HDNode with the path `m/44'/60'/0'` of the
Airnode wallet.
