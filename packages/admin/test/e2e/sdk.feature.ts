...
it.only('provides same API', async () => {
  ...
  const adminApi = difference(Object.keys(admin), ['deriveEndpointId']).sort();

  expect(sdkApi).toEqual(adminApi);
  expect(sdkStaticApi).toEqual(['deriveEndpointId', 'getAirnodeRrp', 'getAirnodeRrpWithSigner']);
});
